...
class TestConvertAllFormats(unittest.TestCase):
    ...
    def _convert(self, fmt):
        ...
        if fmt == "oeb":
            self.assertTrue(os.path.isdir(out_path), f"OEB output directory not created: {out_path}")
            self.assertTrue(os.listdir(out_path), f"OEB output directory is empty: {out_path}")
        else:
            self.assertTrue(os.path.isfile(out_path), f"Output file not created: {out_path}")
            self.assertGreater(os.path.getsize(out_path), 0, f"Output file is empty: {out_path}")
